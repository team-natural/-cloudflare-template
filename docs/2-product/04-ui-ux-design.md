...
| SCR-02 | サービス / 会社紹介ページ（Page） | サービス内容・会社情報の閲覧 | 全訪問者 | Page（採用時。PRD-01 §1-1） |
| SCR-03 | ブログ / お知らせ一覧 | 記事の一覧・絞り込み・検索 | 全訪問者 | F-02-01, F-02-03, F-02-04, F-02-05 |
| SCR-04 | ブログ / お知らせ詳細 | 記事本文の閲覧 | 全訪問者 | F-02-02, F-02-06 |
| SCR-05 | お問い合わせフォーム | 問い合わせ・資料請求の送信 | 全訪問者 | Inquiry（PRD-01 §3-1）, F-06-03 |
| SCR-06 | 商品ページ / カート・決済（軽量 EC 採用時） | 商品情報の閲覧・注文 | 全訪問者 | F-05-01, F-05-02, F-05-03, F-05-04 |
| SCR-07 | 会員登録（マイページ機能採用時） | Member アカウントの新規作成 | 全訪問者 | F-07-01 |
| SCR-08 | ログイン（マイページ機能採用時） | Member としての認証 | 全訪問者 | F-07-02, F-07-03 |
| SCR-09 | マイページ（マイページトップ・登録情報編集、マイページ機能採用時） | 会員情報の確認・編集 | 会員（Member） | F-07-04 |
| SCR-10 | 注文履歴（マイページ機能採用時、かつ Order 採用時のみ・FG-05 連動） | 過去の注文の確認 | 会員（Member） | F-07-05 |
| SCR-11 | パスワード再設定（マイページ機能採用時） | Member パスワードの再設定 | 全訪問者 | F-07-06 |
<!-- SAMPLE END -->
...
| ADM-01 | 管理ダッシュボード | KPI・直近イベント表示 | admin / editor | F-04-01 |
| ADM-02 | 記事（Post）一覧 / 編集 | 記事の作成・編集・公開/非公開切替 | admin / editor | F-04-02 |
| ADM-03 | 固定ページ（Page）一覧 / 編集（採用時のみ） | 固定ページの作成・編集 | admin / editor | F-04-03 |
| ADM-04 | メディア（Media）ライブラリ | 画像等のアップロード・一覧・削除 | admin / editor | F-04-04 |
| ADM-05 | カテゴリ / タグ管理 | Post の分類管理 | admin / editor | F-04-05 |
| ADM-06 | お問い合わせ（Inquiry）一覧 / 対応 | 送信内容の確認・対応状況の更新 | admin | F-04-06 |
| ADM-07 | 管理者（AdminUser）管理 | AdminUser の追加・ロール変更・無効化 | admin | F-04-07 |
| ADM-08 | サイト設定 | サイト名・ロゴ・OGP 既定値等 | admin | F-04-08 |
| ADM-09 | 注文一覧（軽量 EC 採用時のみ） | 注文の一覧・状態確認 | admin | F-05-05 |
