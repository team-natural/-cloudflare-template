...
| Testing | Vitest + Playwright（`Confirmed`） | Unit / Feature = Vitest（Workers 実行環境は `@cloudflare/vitest-pool-workers` で再現）、E2E = Playwright。Architecture テスト（レイヤー境界）は下記 Static Analysis（`eslint-plugin-boundaries`）が担う。テスト戦略の詳細は DEV-03 §4 |
